...
int print_notes(int fd, int uid, char *searchstring){
  ...
  note_length = find_user_note(fd,uid); //This does not include the \n character
  if(note_length == -1) //If end of file reached
    return 0;//return 0

  read(fd, note_buffer, note_length+1); //read note data. the +1 is to compensate for note_length not looking at entire line
